...
public class MyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //InputStream is = context.getResourceAsStream(getServletContext().getRealPath("key.dat"));
        cry = new Crypt(getServletContext().getRealPath("key.dat")); 
        
		Usuario usuario = new Usuario();
		usuario.setName(request.getParameter("user"));
		usuario.setPass(cry.encode(request.getParameter("pass").getBytes()));
		usuario.setPass2(cry.encode(request.getParameter("pass2").getBytes()));
